...
class CodestartsTest extends PlatformAwareTestBase {
    ...
    void prepareProjectTestGradle() throws IOException {
        final CodestartProject codestartProject = Codestarts.prepareProject(new CodestartInput(getPlatformDescriptor(),
                Collections.emptyList(), false, Collections.singletonMap("buildtool.name", "gradle")));
        assertThat(codestartProject.getBuildTool()).extracting(Codestart::getResourceName)
                .isEqualTo("codestarts/base/buildtool/gradle");
    }

    @Test
    void prepareProjectTestKotlin() throws IOException {
        final CodestartProject codestartProject = Codestarts.prepareProject(new CodestartInput(getPlatformDescriptor(),
                Collections.singletonList(AppArtifactKey.fromString("io.quarkus:quarkus-kotlin")),
                false, Collections.emptyMap()));
        assertThat(codestartProject.getLanguage()).extracting(Codestart::getResourceName)
                .isEqualTo("codestarts/base/language/kotlin");
    }

    @Test
    void prepareProjectTestScala() throws IOException {
        final CodestartProject codestartProject = Codestarts.prepareProject(new CodestartInput(getPlatformDescriptor(),
                Collections.singletonList(AppArtifactKey.fromString("io.quarkus:quarkus-scala")),
                false, Collections.emptyMap()));
        assertThat(codestartProject.getLanguage()).extracting(Codestart::getResourceName)
                .isEqualTo("codestarts/base/language/scala");
    }

    @Test
    void prepareProjectTestConfigYaml() throws IOException {
        final CodestartProject codestartProject = Codestarts.prepareProject(new CodestartInput(getPlatformDescriptor(),
                Collections.singletonList(AppArtifactKey.fromString("io.quarkus:quarkus-config-yaml")),
                false, Collections.emptyMap()));
        assertThat(codestartProject.getConfig()).extracting(Codestart::getResourceName)
                .isEqualTo("codestarts/base/config/config-yaml");
    }

    @Test
    void prepareProjectTestResteasy() throws IOException {
        final CodestartProject codestartProject = Codestarts.prepareProject(new CodestartInput(getPlatformDescriptor(),
                Collections.singletonList(AppArtifactKey.fromString("io.quarkus:quarkus-resteasy")),
